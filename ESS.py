...
topic_ems = "EMS/ESS"
topic_ess = "ESS/EMS"
fallback_file = "fallback_data.csv"
...
def resend_fallback_data(client):
    ...
    if unsent_data:
        ...
        for message in unsent_data:
            result = client.publish(topic_ess, message)
            if result.rc != mqtt.MQTT_ERR_SUCCESS:
                print(f"Failed to resend: {message}")
                return False
        clear_fallback_data()
...
def on_message(client, userdata, msg):
    ...
    # Validate the data: Log a warning if values are out of the 10-90 range
    if not (10.0 <= temperature <= 50.0):
        logging.warning(f"Temperature {temperature} out of range (10-90)!")
    ...
    result = client.publish(topic_ess, response_message)

    # Check if publishing failed, and store locally if it did
    if result.rc == mqtt.MQTT_ERR_SUCCESS:
        print(f"Published to EMS: {response_message}")
        resend_fallback_data(client)  # Try to resend any unsent data if connection is restored
    else:
        print(f"Failed to publish message: {response_message}. Storing in fallback.")
        unsent_data = load_fallback_data()
        unsent_data.append(response_message)
